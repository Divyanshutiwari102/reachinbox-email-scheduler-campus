...
export default function SearchableEmailList({ type, searchTerm }: Props) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (searchTerm.trim() === '') {
          ...
          // Searching
          const res = await apiFetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/emails/search?q=${encodeURIComponent(searchTerm)}&type=${type}`);
          if (!res.ok) {
            throw new Error(`Request failed with status ${res.status}`);
          }
          const fetchedData: Email[] = await res.json();
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1 min-w-0">
            <input
              ...
              value={searchTerm}
              onChange={(e) => {}} // controlled by parent, so no-op
            />
  ...
  const filteredEmails = emails.filter((email) => {
    ...
    if (type === 'archived') {
      ...
      // In the searchable version, we always apply the filter when type is sent, regardless of search,
      // because the search term is controlled externally and we want to respect the filter.
      if (type === 'sent' && filter !== 'All') {
